<?php
	/*
	*	MSSQL_Database is a class that performs handling of connections and various querying for a Microsoft SQL server database.
	*
	*	Dependencies:
	*		The PHP mssql library
	*	
	*	Functions:
	...
	*	(11) ms_escape_string - a customized function for escaping data.
	*
	...
	*/
	...
	{	
		...
		protected $quoteFieldTypes = array("date", "datetime", "time", "varchar", "nvarchar", "char", "text");
		...
		*/
		public function query($query, $resultAsArray=null, $returnQueryOnly=null)
		{
			...
			//instantiate the mssql object
			$conn = mssql_connect($this->hostname, $this->user, $this->password);
			
			//check connection, returning an error if necessary.
			if (false === $conn) 
			{
				$resultArr['status'] = "error";	
				$resultArr['errors'] = array("Connect Error: " . mssql_get_last_message());
			...
			else
			{
				...
				else
				{
					...
					{
						$addedIdentity = mssql_fetch_assoc(mssql_query("SELECT SCOPE_IDENTITY() AS AddedIdentity", $conn));
		...
		*/
		public function transaction($queries, $resultAsArray=null)
		{
			...
			{
				//query the database
				$queryResult = mssql_query($query, $conn);
			...
			{
				//rollback
				$rollbackStmt = "ROLLBACK TRAN";
				mssql_query($rollbackStmt, $conn);
				$resultArr['status'] = "error";
			...
			{
				//commit
				$commitStmt = "COMMIT TRAN";
				mssql_query($commitStmt, $conn);
				$resultArr['status'] = "success";
		...
		*/
		public function getColumnInformation($table, $returnArray=null, $justColumnNames=null)
		{
			...
			{
				$fieldArray = array();
				if ("success" == $columnsResult['status'] and mssql_num_rows($columnsResult['result']))
				{
					while ($field = mssql_fetch_assoc($columnsResult['result']))
					{
		...
		*/
		private function processFieldDataForInsert($table, $data)
		{
            ...
            if ($columnsResult and mssql_num_rows($columnsResult))
            {
            	$errors = array();
				$processedFieldArray = array();
				
            	while ($field = mssql_fetch_assoc($columnsResult))
                {
