<?php
	/*
	*	MYSQL_Database is a class that performs handling of connections and various querying for a mysql database.
	*
	*	Dependencies:
	...
	{	
		...
		*/
		public function query($query, $resultAsArray=null, $returnQueryOnly=null)
		{
			...
			//instantiate the mysql obj
			$conn = mysql_connect($this->hostname, $this->user, $this->password);
			
			//check connection, returning an error if necessary.
			if (false === $conn) 
			{
				$resultArr['status'] = "error";	
				$resultArr['errors'] = array("Connect Error: " . mysql_error());
			...
			else
			{
				...
				else
				{
					...
					if (mysql_insert_id())
					{
						$resultArr['result'] = mysql_insert_id();	
					}
					else
					{
		...
		*/
		public function getColumnInformation($table, $returnArray=null, $justColumnNames=null)
		{
			...
			{
				$fieldArray = array();
				if ("success" == $columnsResult['status'] and mysql_num_rows($columnsResult['result']))
				{
					while ($field = mysql_fetch_assoc($columnsResult['result']))
					{
		...
		*/
		private function processFieldDataForInsert($table, $data)
		{
            ...
            if ($columnsResult and mysql_num_rows($columnsResult))
            {
            	$errors = array();
				$processedFieldArray = array();
				
            	while ($field = mysql_fetch_assoc($columnsResult))
                {
